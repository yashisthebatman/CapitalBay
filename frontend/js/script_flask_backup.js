...
// **** THIS FUNCTION WAS UPDATED for active class logic ****
function updateNavigation() {
    ...
    document.querySelectorAll('nav a').forEach(link => {
        ...
        // Special cases for parent highlighting or specific active states
        if (currentPage === 'startup-detail.html' && linkHref === 'index.html') {
            link.classList.add('active'); // Keep Browse active when viewing details
        }
        if (currentPage === 'my-startup.html' && linkHref === 'my-startup.html') {
            link.classList.add('active'); // Make "Update Profile" active
        }
        // **** NEW: Make "Analytics" active ****
        if (currentPage === 'analytics.html' && linkHref === 'analytics.html') {
            link.classList.add('active');
        }
    });
}
...
function initMyStartupPage() {
    ...
    const yearsOperatingInput = document.getElementById('edit-years_operating');
    ...
    if (!profileForm || !financialsForm || !financialHistoryContainer || !addYearButton || !yearsOperatingInput || !profileMessageArea || !financialsMessageArea) {
         ...
         if (mainArea) { mainArea.innerHTML = '<p class="message error-message">Error initializing dashboard page.</p>'; }
         return;
    }
    ...
    // Function to update Years Operating Input based on record count
    function updateYearsOperatingBasedOnFinancialRecords() {
     ...
     // Function to add financial year input block (Edit Page)
     const addFinancialYearEditInput = (yearData = null) => {
        ...
        if(removeButton) {
             removeButton.addEventListener('click', (e) => {
                 ...
                 if(groupToRemove) { groupToRemove.remove(); updateYearsOperatingBasedOnFinancialRecords(); }
             });
        }
    };
    ...
    async function loadMyStartupData() {
        ...
        if (profileResult.ok && profileResult.data) {
            ...
            profileForm.years_operating.value = startup.years_operating ?? ''; // Load initial from DB
            ...
            currentFinancialData = startup.financial_history || [];
             if (Array.isArray(currentFinancialData) && currentFinancialData.length > 0) {
                  ...
                  currentFinancialData.forEach(yearData => addFinancialYearEditInput(yearData));
             } else {
                 addFinancialYearEditInput();
                 financialsMessageArea.innerHTML = '<p><small>No financial history saved yet. Add records below.</small></p>';
             }
            updateYearsOperatingBasedOnFinancialRecords(); // Update count after populating
        } else {
    ...
    financialsForm.addEventListener('submit', async (e) => {
         ...
         if (result.ok) {
             ...
             updateYearsOperatingBasedOnFinancialRecords(); // Update count display
         } else {
...
function initAnalyticsPage() {
    const tableBody = document.getElementById('analytics-investor-table-body');
    const messageArea = document.getElementById('analytics-message-area'); // Optional message area
    ...
    async function loadAnalytics() {
         tableBody.innerHTML = '<tr><td colspan="2" class="text-center loading">Loading investor data...</td></tr>';
         if (messageArea) messageArea.innerHTML = ''; // Clear previous messages

         const result = await apiCall('/my-startup/analytics', 'GET', null, true); // Requires auth

          if (result === null) { // Auth failed or redirect happened
              // Error/redirect is handled by apiCall, maybe display message if element exists
               if (messageArea) messageArea.innerHTML = '<p class="message error-message">Could not authenticate to load analytics.</p>';
               tableBody.innerHTML = '<tr><td colspan="2" class="text-center error-message">Authentication failed.</td></tr>';
              return;
          }

         if (result.ok && result.data && result.data.interested_investors) {
             const investors = result.data.interested_investors;
             tableBody.innerHTML = ''; // Clear loading row

             if (Array.isArray(investors) && investors.length > 0) {
                 investors.forEach(inv => {
                     const row = tableBody.insertRow();
                     row.insertCell().textContent = inv.name || 'N/A';
                     const emailCell = row.insertCell();
                     if (inv.email) {
                         emailCell.innerHTML = `<a href="mailto:${inv.email}">${inv.email}</a>`;
                     } else {
                         emailCell.textContent = 'N/A';
                     }
                     // Add more cells here if needed (e.g., date)
                 });
             } else {
                 tableBody.innerHTML = '<tr><td colspan="2" class="text-center">No investors have expressed interest yet.</td></tr>';
             }
         } else {
             const errorMsg = result.error || 'Failed to load analytics data.';
             console.error("Analytics load error:", errorMsg);
              if (messageArea) messageArea.innerHTML = `<p class="message error-message">${errorMsg}</p>`;
              tableBody.innerHTML = `<tr><td colspan="2" class="text-center error-message">${errorMsg}</td></tr>`;
         }
    }

    loadAnalytics(); // Load data when page initializes
}


// --- Page Initialization ---
// **** UPDATED to include analytics page ****
    ...
    checkAuthStatus().then(() => {
        ...
        if (bodyId === 'page-login') { initLoginPage(); }
             ...
             if (!currentUser || currentUser.user_type !== 'startup') {
                  console.log("User not logged in as startup for profile page, redirecting...");
                  window.location.href = 'login.html';
                 return;
             }
            initMyStartupPage();
        }
        // **** NEW: Initialize analytics page ****
        else if (bodyId === 'page-analytics') {
             if (!currentUser || currentUser.user_type !== 'startup') {
                  console.log("User not logged in as startup for analytics page, redirecting...");
                  window.location.href = 'login.html';
                 return;
             }
            initAnalyticsPage();
        ...
        // **** END NEW ****
         ...
         updateNavigation(); // Update nav after page-specific logic runs
